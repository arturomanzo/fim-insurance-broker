 ...
 *
 * Should be called once per day (e.g., via Vercel Cron or an external scheduler).
 * Sends reminder emails for policies expiring in exactly 60, 30, and 7 days.
 *
 * Protected by CRON_SECRET environment variable (passed as Bearer token).
 */
...
function buildReminderEmail(policy: Policy, daysLeft: number): string {
  ...
  const urgency = daysLeft <= 7 ? '⚠️ Urgente: ' : daysLeft <= 30 ? '📅 ' : '📬 '
...
<body style="font-family:system-ui,sans-serif;background:#f8fafc;margin:0;padding:20px;">
  <div style="max-width:560px;margin:0 auto;background:white;border-radius:12px;overflow:hidden;box-shadow:0 4px 6px rgba(0,0,0,0.07);">
    <div style="background:${daysLeft <= 7 ? '#dc2626' : daysLeft <= 30 ? '#ea580c' : '#0f2d6b'};padding:28px 32px;">
...
export async function GET(req: NextRequest) {
  ...
  const results: { days: number; sent: number; errors: number }[] = []
    ...
    for (const policy of policies) {
      try {
        if (resend) {
          ...
          console.log(`[DEV] Reminder ${days}d: ${policy.clientEmail} — ${policy.tipo} (${policy.numeroPolizza})`)
          sent++
        }
      } catch (err) {
